...
class AboutHashes < Neo::Koan
  ...
  def test_default_value_is_the_same_object
    ...
    hash[:one] << "uno"
    hash[:two] << "dos"
    # Puzzled here, why is it not...?
    assert_equal ["uno", "dos"], hash[:one] # ["uno"], hash[:one]
    assert_equal ["uno", "dos"], hash[:two] # ["dos"], hash[:two]
    assert_equal ["uno", "dos"], hash[:three]  # [], hash[:three]
    # BECAUSE WE'RE SETTING THE DEFAULT VALUE --> the default is whatever is in the array,
    # it defaults to the array because no key/value pairs have yet been set
    # we're accessing the same Array that acts as the default
